...
contract TestFundProject is Test {
	...
	FundRaffleMoodNft fundRaffleMoodNft;
	...
	function setUp() external {
		...
		(fundProject, helperConfig, fundRaffleMoodNft) = deployer.run();
	...
	function testCanFundProject() public {
		...
		assert(
			keccak256(abi.encodePacked(fundRaffleMoodNft.name())) ==
				keccak256(abi.encodePacked((NFT_NAME)))
		);
		assert(
			keccak256(abi.encodePacked(fundRaffleMoodNft.symbol())) ==
